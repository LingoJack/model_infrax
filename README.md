...
│           ├── copy.template   # 复制工具模板
│           └── encode.template # 编码工具模板
├── model/                      # 数据模型定义
├── parser/                     # SQL 解析器
...
模板文件已使用 Go 的 `embed` 指令嵌入到二进制文件中，无需外部模板文件即可运行。模板文件位于 `generator/template/` 目录下：

- **`po.template`**：生成 GORM 实体模型，包含完整的标签和注释
- **`dto.template`**：生成查询和传输用的 DTO 结构体
- **`dao.template`**：生成完整的数据访问层，包含 CRUD 操作
- **`itea-go/`**：itea-go 框架专用模板
  - **`po.template`**：itea-go 风格的 PO 结构体
  - **`dto.template`**：itea-go 风格的 DTO 结构体
  - **`dao.template`**：itea-go 风格的 DAO 层
- **`tools/`**：工具函数模板
  - **`ptr.template`**：生成指针操作工具函数
  - **`str.template`**：生成字符串处理工具函数
  - **`copy.template`**：生成对象复制工具函数
  - **`encode.template`**：生成编码处理工具函数

### 嵌入式模板优势

✅ **单文件部署**：模板文件嵌入到二进制中，无需额外分发模板文件  
✅ **运行时高效**：直接从内存读取模板，无文件 I/O 开销  
✅ **版本一致性**：模板与代码版本同步，避免模板版本不匹配问题  
✅ **安全性**：模板文件不可外部修改，防止意外篡改
...
你可以修改 `generator/template/` 目录下的模板文件来自定义生成的代码格式。修改后需要重新编译程序：

- **修改 PO 结构体**：编辑 `generator/template/po.template`
- **修改 DTO 结构体**：编辑 `generator/template/dto.template`
- **修改 DAO 方法**：编辑 `generator/template/dao.template`
- **修改框架模板**：编辑 `generator/template/itea-go/` 下的模板
- **修改工具函数**：编辑 `generator/template/tools/` 下的模板
...
# 构建结果：
# - jen：可执行二进制文件
# - pack/jen.zip：包含配置和脚本的完整发布包
```

**构建脚本说明**：
- 自动编译二进制文件
- 复制必要的配置文件（application.yml、schema.sql、install.sh）
- 复制 AI 提示文件（prompt/ 目录）
- 创建输出目录结构
- 生成发布包
