...
// mysqlTable 对应 `show table status` 查询结果
...
// mysqlField 对应 `show full fields` 查询结果
...
// InitDatabase 初始化 database 模式配置
// 必须在 conf.InitWithPath 之后调用
func InitDatabase() error {
	if err := loadFilterConfig(); err != nil {
		return err
	}
	logger.Infof("[InitDatabase] database 模式初始化完成, all=%v, tableNames=%v", filterAll, filterTableNames)
	return nil
}

// ParseDatabase 连接数据库解析所有表结构
func ParseDatabase(ctx context.Context) (schemas []model.Schema, err error) {
	var tables []mysqlTable
	err = infra.ExecSql(ctx, &tables, "show table status")
	if err != nil {
	...
	schemas = lo.Map(tables, func(table mysqlTable, index int) model.Schema {
		...
		err = infra.ExecSql(ctx, &fields, "show full fields from ?", tableName)
		if err != nil {
			panic(err)
		}
		...
		err = infra.ExecSql(ctx, &mysqlIndexes, "show index from ?", tableName)
		if err != nil {
			panic(err)
		}
